...
const TableSkeleton = () => (
  <div className="space-y-2">
    ...
    {[...Array(3)].map((_, i) => (
      <Skeleton key={i} className="h-12 w-full" /> /* Rows */
    ))}
  </div>
);
...
function NotionIntegrationsPage() {
  ...
  const [deleteMutationStatus, deleteIntegration] = useAtom(deleteUserNotionIntegrationMutationAtom);
  ...
  React.useEffect(() => {
    if (deleteMutationStatus.isSuccess) {
      ...
      // Query invalidation is handled by the atom's onSuccess
       ...
       toast({
        title: "エラー",
        description: (deleteMutationStatus.error as Error).message || "Notion連携の削除に失敗しました。",
        variant: "destructive",
      });
      setShowDeleteDialog(false);
      setSelectedIntegration(null);
    }
  }, [deleteMutationStatus.isSuccess, deleteMutationStatus.isError, deleteMutationStatus.error, toast]);
    ...
    <>
      ...
      {data && data.length > 0 ? (
              ...
              <TableRow key={integration.id}>
                ...
                <TableCell className="text-right">
                  <Button
                    ...
                    disabled={deleteMutationStatus.isPending && selectedIntegration?.id === integration.id}
                  >
                    <Trash2 className="mr-1 h-4 w-4" />
                    {deleteMutationStatus.isPending && selectedIntegration?.id === integration.id ? "削除中..." : "削除"}
                  </Button>
                </TableCell>
              </TableRow>
            ))}
        ...
        <div className="text-center py-10">
          ...
          <h3 className="mt-2 text-sm font-medium text-gray-900">まだNotion連携が登録されていません。</h3>
          <p className="mt-1 text-sm text-gray-500">「新しいNotion連携を登録」ボタンから最初の連携を登録しましょう。</p>
          <div className="mt-6">
            <Link href="/notion-integrations/new">
              <Button>
                <PlusCircle className="mr-2 h-4 w-4" />
